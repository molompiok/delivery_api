...
export default class VoyageService {
  private static readonly pendingBookingHoldMinutes = 5

  private getPendingHoldAnchor(booking: any) {
    return booking.updatedAt || booking.createdAt
  }

  private isPendingHoldActive(booking: any) {
    if (booking.status !== 'PENDING') {
      return false
    }

    const holdAnchor = this.getPendingHoldAnchor(booking)
    if (!holdAnchor || typeof holdAnchor.plus !== 'function') {
      return false
    }

    return (
      holdAnchor.plus({ minutes: VoyageService.pendingBookingHoldMinutes }).toMillis() >
  ...
  async getSeats(
    ...
    const confirmedReservedSeatsArray = overlappingBookings
      .filter((booking) => booking.status === 'CONFIRMED')
      .flatMap((booking) => booking.seatsReserved || [])
    const pendingReservedSeatsArray = overlappingBookings
      .filter((booking) => this.isPendingHoldActive(booking))
      .flatMap((booking) => booking.seatsReserved || [])

    const confirmedReservedSeats = Array.from(new Set(confirmedReservedSeatsArray))
    const pendingReservedSeats = Array.from(new Set(pendingReservedSeatsArray))
    const reservedSeatsArray = [...confirmedReservedSeats, ...pendingReservedSeats]
    ...
    return {
      totalCapacity,
      reservedCount,
      confirmedReservedCount: confirmedReservedSeats.length,
      pendingReservedCount: pendingReservedSeats.length,
      availableCount: Math.max(0, totalCapacity - reservedCount),
      reservedSeats,
      confirmedReservedSeats,
