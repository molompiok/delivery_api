...
export const confirmationRuleSetSchema = vine.object({
  photo: vine.array(confirmationRuleItemSchema).optional(),
  code: vine.array(confirmationRuleItemSchema).optional(),
})

export const itemValidationRulesSchema = vine.object({
  pickup: confirmationRuleSetSchema.optional(),
  delivery: confirmationRuleSetSchema.optional(),
  service: confirmationRuleSetSchema.optional(),
})
...
export const createOrderSchema = vine.object({
  ...
  transit_items: vine
    .array(
      vine.object({
        ...
        packaging_type: vine
          .enum(['box', 'fluid', 'person', 'BOX', 'FLUID', 'PERSON'] as const)
          .transform((v) => v.toLowerCase() as 'box' | 'fluid' | 'person')
          .optional(),
        ...
        requirements: vine
          .array(
            vine
              ...
              .transform((v) => v.toLowerCase() as any)
          )
          .optional(),
        metadata: vine.any().optional(),
        validation_rules: itemValidationRulesSchema.optional(),
      })
    )
    .optional(),
  assignment_mode: vine
    .enum(['GLOBAL', 'INTERNAL', 'TARGET', 'global', 'internal', 'target'] as const)
    .transform((v) => v.toUpperCase() as 'GLOBAL' | 'INTERNAL' | 'TARGET')
    .optional(),
  ref_id: vine.string().trim().optional(),
  priority: vine
    .enum(['LOW', 'MEDIUM', 'HIGH', 'low', 'medium', 'high'] as const)
    .transform((v) => v.toUpperCase() as 'LOW' | 'MEDIUM' | 'HIGH')
    .optional(),
  metadata: vine.any().optional(),
  pricing_data: vine.any().optional(),
  priceOverrides: vine.record(vine.number()).optional(),
  template: vine
    .enum(['COMMANDE', 'MISSION', 'VOYAGE', 'commande', 'mission', 'voyage'] as const)
    .transform((v) => v.toUpperCase() as 'COMMANDE' | 'MISSION' | 'VOYAGE')
    .optional(),
  ...
  paymentTrigger: vine
    .enum(['BEFORE_START', 'ON_DELIVERY', 'PROGRESSIVE', 'ON_ACCEPT'] as const)
    .optional(),
  payment_trigger: vine
    .enum(['BEFORE_START', 'ON_DELIVERY', 'PROGRESSIVE', 'ON_ACCEPT'] as const)
    .optional(),
})
...
export const addActionSchema = vine.object({
  type: vine
    .enum(['pickup', 'delivery', 'service', 'PICKUP', 'DELIVERY', 'SERVICE'] as const)
    .transform((v) => v.toLowerCase() as 'pickup' | 'delivery' | 'service'),
  quantity: vine.number().min(0).optional(),
  ...
  transit_item: vine
    .object({
      ...
      dimensions: vine
        ...
        .optional(),
      requirements: vine
        .array(vine.enum(['froid', 'fragile', 'dangerous', 'sec'] as const))
        .optional(),
      metadata: vine.any().optional(),
      validation_rules: itemValidationRulesSchema.optional(),
    })
  ...
  confirmation_rules: confirmationRuleSetSchema.optional(),
  metadata: vine.any().optional(),
})
...
export const addStopSchema = vine.object({
  ...
  reversal_amount: vine.number().optional(),
  include_withdrawal_fees: vine.boolean().optional(),
  delivery_fee: vine.number().optional(),
  metadata: vine.any().optional(),
